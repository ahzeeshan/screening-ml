B1;95;0c%This code will take the numner of neurons and do the calcs
%%
clear all
%close all
%clc
...
for coeff_num = 1:num_coeffs
    ...
    tr_perf_reqd = 0.01;
    reg_tr_reqd=0.9;
    Rsq_val_reqd = -inf; Rsq_tt_reqd = 0.7;
    Rsq_tr_reqd = 0.85;
    max_index = 1000;
    ...
    while ( hidden_layer_size==hidden_layer_av(coeff_num) ) || ( length(find(ind_out==0))<10 && (hidden_layer_size <= length(feature_list{coeff_num})) ) 
        ...
        parfor random_weights = 1:sample_test
            ...
            while(bool_var)
                ...
                Rsq_tt = 1 - sum((ytrain(tr.testInd) - out_train(tr.testInd)).^2)/sum((ytrain(tr.testInd) - mean(ytrain(tr.testInd))).^2);
                ...
                bool_var = (Rsq_tt < Rsq_tt_reqd) || (Rsq_tr < Rsq_tr_reqd)
                if tr.tperf(end)<tt_min %&& tr_perf<tr_perf_min
                    tt_min = tt_perf;
                    tr_perf_min = tr.perf(end);
                    net_min = net;
                    tr_min = tr;
                end
...
toc;
save(strcat(lattice,'_layerfinal_resultsp1.mat'),'net_storage_complete','index_out_coeffs','tr_storage_complete')
