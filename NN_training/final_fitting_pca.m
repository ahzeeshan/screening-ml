...
load(strcat(lattice,'_results_pca.mat'))
...
hidden_layer_size = hidden_layer_av %you can provide a row vector which can represent if we need multiple hidden layers
...
Rsq_tr_reqd = 0.8;
max_index = 1000;
index_out = zeros(sample_test, 1);
net_storage = cell(sample_test,1);
tr_storage = cell(sample_test,1);

parfor random_weights = 1:sample_test
    random_weights
    test_min = 1; Rsq_val_max = -400;
    tr_perf_min = 1;
    net_min = [];
    bool_var = 1;
    index_watch = 1;
    ...
    while((bool_var))
        ...
        %test_perf = mse(net,ytrain(tr.valInd),out_train(tr.valInd));
        ...
        %Rsq_val = 1 - tr.vperf(end)*numel(ytrain(:,tr.valInd))/sum(sum((ytrain(:,tr.valInd) - mean(ytrain(:,tr.valInd))).^2));
        %Rsq_tr = 1 - tr.perf(end)*numel(ytrain(:,tr.trainInd))/sum(sum((ytrain(:,tr.trainInd) - mean(ytrain(:,tr.trainInd))).^2));
        %Rsq_test = 1 - tr.tperf(end)*numel(ytrain(:,tr.testInd))/sum(sum((ytrain(:,tr.testInd) - mean(ytrain(:,tr.testInd))).^2));
        Rsq_val = 1 - sum((ytrain(:,tr.valInd) - out_train(:,tr.valInd)).^2,2)./sum((ytrain(:,tr.valInd) - mean(ytrain(:,tr.valInd),2)).^2,2);
        Rsq_tr = 1 - sum((ytrain(:,tr.trainInd) - out_train(:,tr.trainInd)).^2,2)./sum((ytrain(:,tr.trainInd) - mean(ytrain(:,tr.trainInd),2)).^2,2);

        bool_var = (min(Rsq_val) < Rsq_val_reqd) || (min(Rsq_tr) < Rsq_tr_reqd);
        if mean(Rsq_val)>Rsq_val_max %&& tr_perf<tr_perf_min
            Rsq_val_max =  Rsq_val;
            %tr_perf_min = tr_perf;
            net_min = net;
            tr_min = tr;
        ...
        if index_watch>=max_index % this is just for the case when the iterations do not converge even after 1000 runs ....
            index_out(random_weights) = 1;
            break;
        end
        index_out(random_weights) = 0; % Flag for whether it found within 1000 iters 0<->yes
...
ngoodnets = length(find(index_out==0)) ;
ngoodnets

toc;
save(strcat(lattice,'_final_results_pca.mat'),'net_storage_complete','index_out_coeffs','tr_storage_complete')
