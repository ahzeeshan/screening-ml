...
num_kfolds = 5;
mse_size = zeros(num_coeffs, length(hidden_layer_size_range));
...
for coeff_num = 1:1:num_coeffs % for all the coefficients
    ...
    for random_kfold=1:1:num_kfolds
        random_kfold
        % running iterations for hidden layer size
        for layer_size = hidden_layer_size_range
            layer_size
            fun = @(XTRAIN,ytrain,XTEST) neural_net(XTRAIN,ytrain,XTEST,layer_size);
            mse_size(coeff_num, random_kfold, layer_size-1) = crossval('mse',x',t','Predfun',fun,'kfold',k);
        end
        [err,ind_err] = min(mse_size(coeff_num,:));
        hidden_layer_min(coeff_num,random_kfold) = ind_err + 1;
    end
end
