...
load('vol_ratio_cubic.mat');
load('cubic_net_storage_complete24Jul.mat');
load('cubic_test_err_complete24Jul.mat');
load('cubic_index_partial_trained24Jul.mat')
lattice = 'cubic';
load(strcat('coefficient_',lattice,'.mat'))
load(strcat(lattice,'-training-features-MATLAB.mat'))
load(strcat(lattice,'-non-training-feature.mat'))
...
load('hidden_layersize_cubic.mat')
num_samples = 1000;
cubic_nt = cubic_nt'; 
...
for nt = 1:1:num_coeffs
    feature_arr = feature_list{nt};   

    X1 = X_mat(:,feature_arr);
    non_training{nt} = cubic_nt(:,feature_arr)';

    x = X1'; %Doing transpose to feed into Neural network

    %Mapminmax starts below to map x from [-1,1]

    [x, tot_inp_recover{nt}] = mapminmax(x); 
    ...
    non_training{nt} = mapminmax('apply',non_training{nt},tot_inp_recover{nt});
    net_storage= net_storage_complete{nt};
    t = coeffs(:,nt)'; % t here refers to the target in ANNs
    [t, t_recover{nt}] = mapminmax(t); %Mapping the coefficients as well in [-1,1]

for num_samp = 1:num_samples 
    net = net_storage{num_samp};
    Coeffs_mat = net(non_training{nt});
    Coeffs_mat = mapminmax('reverse',Coeffs_mat,t_recover{nt});
    Coeffs_cell{num_samp,nt} = Coeffs_mat;
end
end
%%
N_choose = 18;
