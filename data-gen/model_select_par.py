...
def multi_run_wrapper(args):
    return get_best_model_for_data(*args)
...
'''
#    print([(X,y[:,coeff], these_models_and_parameters, 2, lattice+'_coeff'+str(coeff)) for coeff in range(ncoeffs)]    
    p = Pool(6)
    p.map(multi_run_wrapper, [(X,y[:,coeff], these_models_and_parameters, 2, lattice+'_coeff'+str(coeff)) for coeff in range(ncoeffs)])
